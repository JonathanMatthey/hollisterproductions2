...
(function() {
    ...
    if (typeof jQuery != 'undefined') {
        jQuery.fn.requestFullScreen = function(callback) {
...
mejs.MediaFeatures = {
	init: function() {
		...
		t.hasTouch = ('ontouchstart' in document.documentElement); // ('ontouchstart' in window);
			...
			case 'youtube':
				...
				var
					...
					youtubeSettings = {
						...
						pluginId: pluginid,
						videoId: videoId,
						height: height - controlbarHeight, // change by Minh Nguyen
						width: width	
					};				
...
mejs.YouTubeApi = {
	...
	createEvent: function (player, pluginMediaElement, eventName) {
		...
		if (player && player.getDuration) {
			...
			// added by Minh Nguyen to get size of video;
			pluginMediaElement.playbackQuality = obj.playbackQuality = player.getPlaybackQuality();
	...
	createFlash: function(settings) {
		...
		*/
		...
		if (mejs.MediaFeatures.isIE) {
			...
			specialIEContainer.outerHTML = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab" ' +
				'id="' + settings.pluginId +
					...
					//'width="100%" height="100%" style="visibility: visible; ">' +
		...
		settings.container.style.width= "100%";
		settings.container.style.height= "100%";
	},
 ...
 */
		...
		mejs.MediaElementPlayer.prototype = {
			...
			setControlsSize: function() {
				...
				if (railWidth === 0 || !railWidth) {
					...
					railWidth = t.controls.width() - usedWidth - (rail.outerWidth(true) - rail.outerWidth(false)) - 2;
				}
	...
	(function($) {
		...
		var normalHeight = 0, normalWidth = 0;
		$.extend(MediaElementPlayer.prototype, {
							...
							*/
							...
							fullscreenBtn.click(function() {
								if (!t.isFullScreen) { 
									...
									// change by Minh Nguyen;
									t.layers.children('div').each(function() {
										if ($(this).hasClass('fix-size')) return;
										$(this).width('100%').height('100%');
									});
									
									t.setControlsSize();
									t.isFullScreen = true;
									t.container.css({width: '100%', height: '100%'}).requestFullScreen();
								} else {
			...
			exitFullScreen: function() {
				...
				*/
				...
				if (t.pluginType === 'native') {
					...
					t.container.find('embed')
						.width(normalWidth)
						.height(normalHeight - controlbarHeight);
						
					t.media.setVideoSize(normalWidth, normalHeight - controlbarHeight);
				}				

				t.layers.children('div').each(function() {
					if($(this).hasClass('fix-size')) return;
					$(this).width(normalWidth).height(normalHeight);
				});

				t.layers.find(".mejs-annotations").height(0);
				t.layers.find(".mejs-header").width("100%");
